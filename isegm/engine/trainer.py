...
class ISTrainer(object):
    ...
    def training(self, epoch):
        data_loader_iter = iter(self.train_data)
        ...
        tbar = tqdm(total=len(data_loader_iter))  # 进度条
        ...
        self.net.train()
        train_loss = 0.0
        for i, batch_data in enumerate(data_loader_iter):
    ...
    def validation(self, epoch):
        ...
        val_loader_iter = iter(self.val_data)

        tbar = tqdm(total=len(val_loader_iter))
        ...
        self.net.eval()
        for i, batch_data in enumerate(val_loader_iter):
            ...
            if self.is_master:
                for metric in self.val_metrics:
                    # metric.log_states(self.sw, f'{log_prefix}Metrics/{metric.name}', global_step)
                    miou += metric._epoch_iou_sum / metric._epoch_batch_count
                tbar.update(1)
                tbar.set_description(f'Epoch {epoch}, validation loss: {val_loss / (i + 1):.4f}, miou: {miou}')
