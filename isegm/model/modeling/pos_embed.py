...
"""
对模型位置嵌入（positional embedding）进行插值，以适应不同大小的输入图像。
在视觉 Transformer 模型（例如 ViT、SegFormer、SimpleClick 等）中，位置嵌入会受到输入图像分辨率变化的影响，因此需要调整嵌入以匹配新的分辨率。
pos_embed: 模型的原始位置嵌入。
patch_embed: 用于提取图像补丁的模块，包含补丁大小和网格尺寸。
infer_img_size: 推理过程中输入图像的大小（宽度和高度）。
grid_size: 原始嵌入的网格尺寸（高宽补丁数量）
"""
